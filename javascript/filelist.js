 ...
 */
function FilelistEntry(text, value, count)
{
    this.text = text;
    this.value = value;
    this.count = count;
...
function createFilelist(col)
{
    ...
    {
        if (filelist.length == 0 && col != 11) {
            filelist.push(new FilelistEntry(combined[i][0],  combined[i][col], 1));
        } else if (filelist.length == 0 && col == 11) {
            filelist.push(new FilelistEntry(combined[i][0],  combined[i][col], 0));
        } else {
            if (filelist[filelist.length-1].text == combined[i][0]) {
                ...
                // for maintainability index metric increment count 
                if (col == 11) {
                    filelist[filelist.length-1].count += 1;
                }
            } else {
                if (col != 11) {
                    filelist.push(new FilelistEntry(combined[i][0],  combined[i][col], 1));
    ...
    // interate through filelist and change value to average 
    for (var i = 0; i < filelist.length; i++) {
        filelist[i].value = Math.round(filelist[i].value / filelist[i].count)
    };
}
